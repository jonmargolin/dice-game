...
const GameLayout = () => {
  ...
  const handleMessage = (event: MessageEvent) => {
    ...
    if (data) {
      if (!Array.isArray(data.message)) {
        ...
        message.sort(
          (
            a: { rollSum: string; roundNumber: string },
            b: { rollSum: string; roundNumber: string }
          ) => {
        ...
        dispatch({
          type: 'UPDATE_RESULT',
          payload: { data: message, isWin: isWin },
        });
        setModalOpen(false);
      }
    }
  ...
  const rollDice = () => {
    muteRollDice(undefined, {
      onSuccess: (data) => updateGameContext(data),
  ...
  const clearGame = () => {
    mutateClearGame(undefined, {
      onSuccess: () => clearContextGame(),
      onError: (err) => handelError(err.message),
    });
  };
  const logoutContext = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <Modal isOpen={isModalOpen} style={customStyles} ariaHideApp={false}>
        <EndGameModal
          reRoll={() => rollDice()}
          timer={timerRef.current as number}
          skipTurn={() => skipTurn()}
        />
      </Modal>
      <ToastContainer />
