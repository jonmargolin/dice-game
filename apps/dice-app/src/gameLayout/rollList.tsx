...
const RollList = ({rollList}:RollListProps) => {
  ...
  if(rollList.length>0){
    rollList.forEach((rollItem, index) =>{
        lisOfRoll.push(<RollItem key={`rollItem${index}${rollItem?.roundNumber}`} rollArray={rollItem.rollList}  sum={rollItem?.rollSum} probability={rollItem?.rollProbability} rollNumber={rollItem?.roundNumber}  />)
    })
    return lisOfRoll
  }
    }
