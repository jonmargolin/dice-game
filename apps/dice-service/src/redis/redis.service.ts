...
export class RedisService {
  ...
  async getAllRollDiceObjects(): Promise<RollDice[]> {
    ...
    const allRollIds = await this.redisRepository.smembers(
      this.allRollDiceSetKey
    );
    ...
    return (await (await multi.exec())
      .flat()
      .filter((item) => item !== null)) as RollDice[];
  }
  ...
  async endRounds(userId: string): Promise<void> {
    ...
    const isUserNotfound = this.checkIfDoseNotIdeExists(
      showResultData.activeUser,
      userId
    );
    if (isUserNotfound) {
  ...
  async gameStatus(): Promise<ShowResult> {
    const showResultExists = await this.isGameExists();
    if (showResultExists) {
      const result = await this.redisRepository.hget(
        this.showResultKey,
        'data'
      );
