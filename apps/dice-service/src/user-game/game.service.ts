...
export class GameService {
  constructor(
    ...
    private readonly updateService: ServerSideUpdateService
  ) {}
  private roll(): number {
  ...
  public rollDice(roundNumber: number, userId: string): RollDice {
    ...
    const rollSum = currentRoll.reduce(
  ...
  public async checkIfRoundEndAndUserExists(userId: string): Promise<boolean> {
    ...
    const userDoseNotExists = await this.redisService.checkIfDoseNotIdeExists(
      isRoundEnded.activeUser,
      userId
    );
